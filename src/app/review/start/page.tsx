    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-gray-700 mb-6">Сначала изучи карточки в наборах и отметь «Знаю»</p>
          <Link href="/decks" className="inline-block px-6 py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition">
            Перейти к наборам
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-100 via-teal-100 to-blue-100 py-8 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <Link href="/review" className="px-4 py-2 bg-white rounded-lg shadow text-gray-700 hover:bg-gray-50 font-medium">← Выход</Link>
          <div className="flex items-center gap-4">
            <div className="text-sm font-medium text-gray-700">{currentIndex + 1} из {cards.length}</div>
            <div className="flex gap-2">
              <span className="px-3 py-1 bg-green-500 text-white rounded-full text-sm font-semibold">✓ {sessionStats.know}</span>
              <span className="px-3 py-1 bg-red-500 text-white rounded-full text-sm font-semibold">✗ {sessionStats.dontKnow}</span>
            </div>
          </div>
        </div>

        <div className="w-full h-2 bg-white/50 rounded-full mb-8 overflow-hidden">
          <div className="h-full bg-gradient-to-r from-green-500 to-teal-600 transition-all duration-300" style={{ width: `${((currentIndex + 1) / cards.length) * 100}%` }} />
        </div>

        <div
          className={`relative mb-8 ${!isFlipped ? 'cursor-pointer' : ''} transition-opacity duration-200`}
          style={{ perspective: '1000px', height: '400px', opacity: isTransitioning ? 0 : 1 }}
            ...
            <div className="absolute w-full h-full" style={{ backfaceVisibility: 'hidden' }}>
              <div className="bg-white rounded-2xl shadow-2xl p-12 h-full flex flex-col justify-center">
                <div className="text-center">
                  ...
                  <p className="text-5xl font-bold text-gray-900 mb-3">{frontText}</p>
                  {cardData.ru_transcription && <p className="text-xl text-blue-600 mb-8 italic">[{cardData.ru_transcription}]</p>}
                  <p className="text-gray-600 text-lg mt-8">👆 Нажми чтобы увидеть ответ</p>
                </div>
              </div>
            </div>
              ...
              <div className="bg-gradient-to-br from-green-500 to-teal-600 rounded-2xl shadow-2xl p-12 h-full flex flex-col justify-center">
                <div className="text-center text-white">
                  ...
                  <div className="flex items-center justify-center gap-4 mb-3">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); speakText(cards[currentIndex].cards.en_text, 'en'); }} className="text-4xl hover:scale-110 transition-transform active:scale-95" title="Прослушать">🔊</button>
                  </div>
                  <div className="mt-8 pt-6 border-t-2 border-white/30">
                    <p className="text-sm text-green-100 mb-2">Перевод:</p>
                    <p className="text-2xl text-white font-medium">🇷🇺 {frontText}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        {isFlipped && (
          <div className="grid grid-cols-2 gap-4">
            <button onClick={() => handleAnswer(false)} className="py-6 bg-red-500 text-white rounded-xl font-bold text-xl hover:bg-red-600 transition shadow-lg active:scale-95">✗ Не знаю</button>
            <button onClick={() => handleAnswer(true)} className="py-6 bg-green-500 text-white rounded-xl font-bold text-xl hover:bg-green-600 transition shadow-lg active:scale-95">✓ Знаю</button>
          </div>
        )}
