...
function Num({ value, suffix = '' }: { value: number; suffix?: string }) {
  ...
  const display: MotionValue<string> = useTransform(spring, (v: number) =>
...
function getDeckStage(s: DeckSrsStats): { label: string; percent: number } {
  ...
  const reviewFraction = (s.youngCount + s.matureCount) / s.total;
  const testFraction = (s.learningCount + s.testingCount + s.youngCount + s.matureCount) / s.total;
  if (reviewFraction > 0.3) return { label: 'Повторение', percent: Math.round(reviewFraction * 100) };
  if (testFraction > 0.1) return { label: 'Тестирование', percent: Math.round(testFraction * 100) };
  return { label: 'Изучение', percent: s.masteryPercent };
}

/* ─── Nav items ─── */
const NAV = [
  { href: '/dashboard-v4', label: 'Главная',    d: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
  ...
  return (
    <aside className="w-52 shrink-0 flex flex-col h-screen sticky top-0 bg-white border-r border-gray-100">
      ...
      <div className="px-5 pt-6 pb-5">
        <Link href="/dashboard-v4" className="flex items-center gap-2.5 group">
      ...
      <div className="px-3 py-4 border-t border-gray-100">
        <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl hover:bg-gray-50 transition cursor-default">
...
function DeckCard({ deck, delay = 0 }: { deck: DeckWithStats; delay?: number }) {
  ...
  const pal = STAGE_PALETTE[stage.label as keyof typeof STAGE_PALETTE] ?? STAGE_PALETTE['Изучение'];
    ...
    return (
      ...
      >
        <Link href={`/decks/${deck.id}`}>
          <motion.div
            whileHover={{ y: -2, boxShadow: '0 12px 32px rgba(5,122,85,0.2)' }}
            transition={{ duration: 0.2 }}
            className="relative overflow-hidden rounded-2xl p-5 h-full cursor-pointer"
            style={{ background: 'linear-gradient(135deg, #057A55 0%, #065f46 100%)' }}
          >
            <motion.div
            ...
            <div className="relative">
              <div className="flex items-start justify-between mb-3">
                <span className="inline-flex items-center gap-1 text-xs font-semibold bg-amber-400/20 text-amber-300 rounded-full px-2.5 py-1">
                  ★ Завершён
                </span>
              </div>
              <h3 className="font-bold text-white text-base leading-snug mb-1">{deck.name}</h3>
              {deck.description && (
                <p className="text-white/50 text-xs line-clamp-1 mb-3">{deck.description}</p>
              )}
    ...
    >
      <Link href={`/decks/${deck.id}`}>
        <motion.div
          ...
          transition={{ duration: 0.2 }}
          className={`bg-white rounded-2xl border-l-4 ${pal.border} border-t border-r border-b border-gray-100 p-5 h-full cursor-pointer group shadow-sm`}
        >
          {/* Icon + badge */}
          <div className="flex items-start justify-between mb-3">
            <div className={`w-8 h-8 ${pal.iconBg} rounded-xl flex items-center justify-center`}>
              <div className={`w-2.5 h-2.5 ${pal.dot} rounded-full`} />
            </div>
            {readyForReview > 0 && (
              <span className="text-xs font-semibold bg-orange-50 text-orange-500 rounded-full px-2.5 py-0.5 border border-orange-100">
                {readyForReview} к повтор.
              </span>
            )}
          </div>

          {/* Name */}
          <h3 className="font-bold text-gray-900 text-[15px] leading-snug mb-1 group-hover:text-[#057A55] transition-colors">
            {deck.name}
          </h3>
          {deck.description && (
            <p className="text-gray-400 text-xs mb-3 line-clamp-1">{deck.description}</p>
          )}
          ...
          {total > 0 && (
            <div className="mt-auto">
              ...
              <div className="w-full h-[5px] bg-gray-100 rounded-full overflow-hidden">
                <motion.div
                  className={`h-full ${pal.bar} rounded-full`}
                  initial={{ width: 0 }}
                  animate={{ width: `${stage.percent}%` }}
                  transition={{ duration: 0.6, ease: 'easeOut', delay: delay + 0.15 }}
                />
              </div>
              <div className="text-[11px] text-gray-400 mt-1">{deck.stats.masteredCount} / {total} слов</div>
            </div>
          )}
        </motion.div>
      </Link>
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="w-20 h-1 bg-gray-100 rounded-full overflow-hidden">
          <motion.div
            className={`h-full rounded-full ${today.completed ? 'bg-[#057A55]' : 'bg-amber-400'}`}
            initial={{ width: 0 }}
            animate={inView ? { width: `${todayPct}%` } : {}}
...
export default function DashboardV4Page() {
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: '#F7F5F0', fontFamily: "'Sora', 'Inter', sans-serif" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Sora:wght@400;500;600;700;800&display=swap');`}</style>

      <Sidebar name={displayName} reviewCount={reviewReady} onSignOut={async () => { await signOut(); router.push('/login'); }} />
            ...
            <>
              {myDecks.length > 0 && (
                <section className="mb-8">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {myDecks.slice(0, 9).map((d, i) => <DeckCard key={d.id} deck={d} delay={0.1 + i * 0.06} />)}
                  </div>
                  {myDecks.length > 9 && (
              ...
              {groupSections.map((g, gi) => (
                <section key={g.id} className="mb-8">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {g.decks.slice(0, 9).map((d, i) => <DeckCard key={d.id} deck={d} delay={0.15 + gi * 0.05 + i * 0.06} />)}
                  </div>
                  {g.decks.length > 9 && (
                    <div className="mt-3 text-center">
