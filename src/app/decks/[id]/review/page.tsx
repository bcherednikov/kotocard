...
export default function DeckReviewPage() {
  ...
  if (loading) {
    return (
      <ImmersiveShell backHref={`/decks/${deckId}`} title="Повторение">
        <div className="flex items-center justify-center h-full">
          <p className="text-gray-500 text-sm">Загрузка повторения...</p>
        </div>
      </ImmersiveShell>
    );
  }
      ...
      <ImmersiveShell backHref={`/decks/${deckId}`} title="Повторение">
        <div className="flex items-center justify-center h-full px-4">
          <div className="text-center">
            ...
            <p className="text-gray-500 text-sm">Сначала изучи карточки и отметь «Знаю»</p>
          </div>
        </div>
      </ImmersiveShell>
    );
  ...
  return (
    <ImmersiveShell
      ...
      progress={progress}
      leftBadge={<Badge variant="green">✓ {sessionStats.know}</Badge>}
      rightBadge={<Badge variant="red">✗ {sessionStats.dontKnow}</Badge>}
    >
            ...
            <div className="absolute w-full h-full" style={{ backfaceVisibility: 'hidden' }}>
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-10 h-full flex flex-col justify-center">
                <div className="text-center">
                  ...
                  {cardData.ru_transcription && <p className="text-lg text-[#057A55] mb-4 italic">[{cardData.ru_transcription}]</p>}
